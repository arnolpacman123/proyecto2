                    ...
                    <div class="logo-area">
                        ...
                        @if(Session::has('mode'))
                            @if(Session::get('mode')['id'] == 'night')
                                <a class="pjax" href="{{ route('welcome') }}"><img id="logo_mode"
                                                                                   src="{{ asset($option_value->light_logo) }}"
                                                                                   alt=""></a>
                            @else
                                <a class="pjax" href="{{ route('welcome') }}"><img id="logo_mode"
                                                                                   src="{{ asset($option_value->dark_logo) }}"
                                                                                   alt=""></a>
                            @endif
                        @else
                            <a class="pjax" href="{{ route('welcome') }}"><img id="logo_mode"
                ...
                <div class="col-lg-6">
                    <div class="header-search-area">
                        <div class="header-searchbox text-center">
                            <input type="text" placeholder="{{ __('Search') }}" id="search" oninput="search()"
                                   autocomplete="off">
                    ...
                    <div class="header-right-section f-right">
                        @if(Auth::check())
                            <div class="upload-btn">
                                @if(Session::has('mode'))
                                    @if(Session::get('mode')['id'] == 'night')
                                        <a class="pjax" href="{{ route('welcome') }}"><img id="home_mode" class="home"
                                                                                           src="{{ asset('frontend/img/white_home.png') }}"
                                                                                           alt=""></a>
                                    @else
                                        <a class="pjax" href="{{ route('welcome') }}"><img id="home_mode" class="home"
                            ...
                            <div class="upload-btn">
                                <div class="notification-menu">
                                    ...
                                    @if(Session::has('mode'))
                                        @if(Session::get('mode')['id'] == 'night')
                                            <a href="#" onclick="notification_unread()" data-toggle="dropdown"
                                               aria-haspopup="true" aria-expanded="false"><img id="notification_mode"
                                                                                               src="{{ asset('frontend/img/white_notification.png') }}"
                                                                                               alt=""></a>
                                        @else
                                            <a href="#" onclick="notification_unread()" data-toggle="dropdown"
                                               aria-haspopup="true" aria-expanded="false"><img id="notification_mode"
                                                                                               src="{{ asset('frontend/img/notification.png') }}"
                                                                                               alt=""></a>
                                        @endif
                                    @else
                                        <a href="#" onclick="notification_unread()" data-toggle="dropdown"
                                    ...
                                    <div class="notification-count">
                                        <span
                                            class="notification_count {{ $notification_count->count() > 0 ? '' : 'd-none' }}">{{ $notification_count->count() }}</span>
                                    </div>
                            ...
                            <div class="upload-btn">
                                @if(Session::has('mode'))
                                    @if(Session::get('mode')['id'] == 'night')
                                        <a class="pjax" href="{{ route('upload') }}"><img id="upload_mode"
                                                                                          class="upload"
                                                                                          src="{{ asset('frontend/img/white_upload.png') }}"
                                                                                          alt=""></a>
                                    @else
                                        <a class="pjax" href="{{ route('upload') }}"><img id="upload_mode"
                                                                                          class="upload"
                                                                                          src="{{ asset('frontend/img/upload.png') }}"
                                                                                          alt=""></a>
                                    @endif
                                @else
                            ...
                            <div class="profile-seeting">
                                <a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><img
                                        class="profile" src="{{ asset(Auth::User()->image) }}" alt=""></a>
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a href="{{ route('profile.show',Auth::User()->slug) }}"
                                       class="pjax dropdown-item">{{ Auth::User()->first_name }} {{ Auth::User()->last_name }}</a>
                                    <a href="{{ route('profile.edit') }}"
                                       class="pjax dropdown-item">{{ __('Edit Profile') }}</a>
                                    ...
                                    <div class="dropdown-border">
                                        ...
                                        <a href="{{ route('latest') }}"
                                           class="pjax dropdown-item">{{ __('Latest') }}</a>
                                        <a href="{{ route('popular') }}"
                                           class="pjax dropdown-item">{{ __('Most View') }}</a>
                                        <a href="{{ route('users') }}"
                                           class="pjax dropdown-item">{{ __('Find Users') }}</a>
                                    </div>
                                    @if(Auth::User()->role_id == 1)
                                        <div class="dropdown-border">
                                            <a href="{{ route('admin.dashboard') }}"
                                               class="dropdown-item">{{ __('Admin Panel') }}</a>
                                        </div>
                                    @endif
                                    <div class="dropdown-border">
                                    ...
                                    <div class="dropdown-border">
                                        ...
                                        @if(Session::has('mode'))
                                            @if(Session::get('mode')['id'] == 'day')
                                                <a href="#" id="mode_action" onclick="mode()"
                                                   class="dropdown-item">{{ __('Night Mode') }}
                                                    <div class="mode night"><i class="far fa-moon"></i></div>
                                                </a>
                                            @endif
                                            @if(Session::get('mode')['id'] == 'night')
                                                <a href="#" id="mode_action" onclick="mode()"
                                                   class="dropdown-item">{{ __('Day Mode') }}
                                                    <div class="mode day"><i class="far fa-sun"></i></div>
                                                </a>
                                            @endif
                                        @else
                                            <a href="#" id="mode_action" onclick="mode()"
