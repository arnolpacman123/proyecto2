...
function darkMode(el, flag = undefined) {
    if (el) {
        const darkModeSwitch = localStorage.getItem('darkModeSwitch');
        const checked = el.getAttribute("checked");
        console.log({darkModeSwitch, checked});
        if (darkModeSwitch !== 'dark' && !el.getAttribute("checked")) {
            localStorage.setItem('darkModeSwitch', 'dark');
        } else {
            localStorage.setItem('darkModeSwitch', 'light');
        }
    }
